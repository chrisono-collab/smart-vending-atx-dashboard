...
# === VendSoft Configuration (Primary Data Source) ===
vendsoft_txn_log = Path('data/vendsoft/usat-transaction-log.xlsx')
vendsoft_sku_map = Path('data/vendsoft/sku-mapping-cleaned.xlsx')
location_mapping_file = Path('location_mapping.csv')

# Check if VendSoft data is available
USE_VENDSOFT = vendsoft_txn_log.exists() and vendsoft_sku_map.exists()

if USE_VENDSOFT:
    print("="*60)
    print("VendSoft Mode: Using unified transaction log")
    print("="*60 + "\n")

    # Initialize VendSoft processor
    processor = VendSoftProcessor(
        transaction_log_path=vendsoft_txn_log,
        sku_mapping_path=vendsoft_sku_map,
        location_mapping_path=location_mapping_file
    )

    # Process transactions (Full YTD - no date filter)
    all_transactions = processor.process_transactions()

    # Generate unmapped report
    unmapped_report = processor.generate_unmapped_report(all_transactions)

    # Print summary
    processor.print_summary()

    # Save outputs
    all_transactions.to_csv('data/processed/master_dashboard_data.csv', index=False)
    if len(unmapped_report) > 0:
        unmapped_report.to_csv('data/processed/unmapped_products_report.csv', index=False)
        print(f"⚠️  Warning: {len(unmapped_report)} unmapped products (see unmapped_products_report.csv)")
    ...
    print(f"✓ Success! VendSoft transaction data processed.")
    ...
    print(f"\n  Output: data/processed/master_dashboard_data.csv")

else:
    print("="*60)
    print("Legacy Mode: Using multi-system processing (Haha AI + Cantaloupe + Nayax)")
    print("="*60 + "\n")
    ...
    # Add Type and mapping_tier columns for backward compatibility with new schema
    all_transactions['Type'] = 'Unknown'
    all_transactions['mapping_tier'] = 'legacy'

    # Reorder columns to match new 12-column schema
    all_transactions = all_transactions[['date', 'location', 'Master_SKU', 'Master_Name', 'Product_Family', 'Type', 'revenue', 'cost', 'quantity', 'profit', 'gross_margin_percent', 'mapping_tier']]
    ...
    print(f"✓ Success! Transaction-level dashboard data generated (Legacy Mode).")
    print(f"\n  Date Range: {date_range}")
    print(f"  Total Revenue: ${total_revenue:,.2f}")
    print(f"  Total Profit: ${total_profit:,.2f}")
    print(f"  Average Margin: {avg_margin:.1f}%")
